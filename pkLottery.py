        ...
        for i in range(0, len(json_response)):
            ...
            if PKId not in PKLottery.had_gotten_pk and PKId != 0:
                ...
                if not OriginRoomId == PKLottery.last_pk_room:
                    ...
                    if True in result:
                        print(f"检测到房间 {OriginRoomId} 的钓鱼操作")
                        continue
                ...
                if json_response2['code'] == 0:
                    print(f"参与房间 {OriginRoomId} 编号 {PKId} 的大乱斗获胜抽奖: {json_response2['data']['award_text']}")
                elif json_response2['code'] == -2 and json_response2['message'] == "您已参加过抽奖":
                    print(f"房间 {OriginRoomId} 编号 {PKId} 的大乱斗获胜抽奖已参与过")
                elif json_response2['code'] == -403 and json_response2['message'] == "访问被拒绝":
                    print(f"参与房间 {OriginRoomId} 编号 {PKId} 的大乱斗获胜抽奖: {json_response2['message']}")
                    print(json_response2)
                else:
                    print(f"房间 {OriginRoomId} 编号 {PKId} 的大乱斗获胜抽奖参与出错: {json_response2}")
